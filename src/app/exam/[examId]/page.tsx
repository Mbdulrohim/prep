...
export default function ExamPage() {
  ...
  const checkExamEligibility = async () => {
    ...
    try {
      ...
      const eligibilityResult = await examAttemptManager.canUserStartExam(user.uid, examId);
      ...
      if (!eligibilityResult.canStart) {
        ...
        if (completedAttempt) {
          ...
          setError(eligibilityResult.reason || 'No exam access found. Please purchase exam access or redeem an access code.');
        }
      } else {
      ...
      <div className="min-h-screen bg-gray-50">
        ...
        <div className="flex items-center justify-center min-h-[80vh]">
          <div className="text-center">
            ...
            <p className="text-lg text-slate-700">Checking exam eligibility...</p>
          </div>
        </div>
      </div>
    );
            ...
            <div className="bg-gray-50 rounded-lg p-4 mb-6">
              ...
              <div className="flex justify-between items-center mb-2">
                ...
                <span className="font-medium">
                  {Math.round((examAttempt.score / examAttempt.totalQuestions) * 100)}%
                </span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-sm text-gray-600">Completed:</span>
                <span className="text-sm">
                  {examAttempt.submittedAt && new Date(examAttempt.submittedAt).toLocaleDateString()}
                </span>
              </div>
            </div>
            <div className="space-y-3">
