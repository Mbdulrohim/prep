...
export async function POST(request: NextRequest) {
  try {
    ...
    const userAccessData = userAccessDoc.exists() ? userAccessDoc.data() : { redeemedCodes: [] };
    ...
    return NextResponse.json({
      success: true,
      message: `Access granted for ${accessCodeData.examCategory} ${accessCodeData.papers.join(' & ')}`,
      accessDetails: {
        examCategory: accessCodeData.examCategory,
        papers: accessCodeData.papers,
        expiryDate: accessCodeData.expiryDate
