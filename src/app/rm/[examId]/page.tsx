...
const RMExamPage: React.FC = () => {
  ...
  const [activeAttempt, setActiveAttempt] = useState<StandaloneRMAttempt | null>(null);
  ...
  const fetchExamData = async (currentUser: User, examId: string) => {
    try {
      ...
      const attempt = await standaloneRMExamManager.getUserActiveAttempt(currentUser.uid, examId);
          ...
          <div className="px-8 py-6">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
              <div className="text-center">
                ...
                <p className="text-lg font-semibold text-gray-900">{formatDuration(exam.timeLimit)}</p>
              </div>
              ...
              <div className="text-center">
                ...
                <p className="text-lg font-semibold text-gray-900 capitalize">{exam.difficulty}</p>
              </div>
            </div>

            {/* Active Attempt Info */}
            {activeAttempt && !activeAttempt.isCompleted && (
              <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-6">
                <div className="flex items-start">
                  <AlertCircle className="h-5 w-5 text-yellow-600 mt-0.5 mr-3" />
                  <div>
                    <h4 className="font-medium text-yellow-900">Resume Previous Attempt</h4>
                    <p className="text-sm text-yellow-700 mt-1">
                      You have an incomplete exam attempt started on{" "}
                      {activeAttempt.startTime.toLocaleDateString()} at{" "}
                      {activeAttempt.startTime.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}.
            ...
            <div className="bg-gray-50 rounded-lg p-6 mb-6">
              <h3 className="font-semibold text-gray-900 mb-4">Exam Instructions</h3>
              <ul className="space-y-2 text-sm text-gray-600">
                <li className="flex items-start">
                  <span className="flex-shrink-0 w-1.5 h-1.5 bg-blue-600 rounded-full mt-2 mr-3"></span>
                  You have {formatDuration(exam.timeLimit)} to complete {exam.totalQuestions} questions
                </li>
                <li className="flex items-start">
                  <span className="flex-shrink-0 w-1.5 h-1.5 bg-blue-600 rounded-full mt-2 mr-3"></span>
                ...
                <li className="flex items-start">
                  <span className="flex-shrink-0 w-1.5 h-1.5 bg-blue-600 rounded-full mt-2 mr-3"></span>
                  You can retake the exam multiple times during your access period
                </li>
              </ul>
            </div>
