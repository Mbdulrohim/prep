...
export const DocumentUpload: React.FC<DocumentUploadProps> = ({ onQuestionsExtracted }) => {
  ...
  const examOptions = [
    { id: 'rn-paper-1', label: 'RN Paper 1', description: 'Registered Nurse Paper 1' },
    { id: 'rn-paper-2', label: 'RN Paper 2', description: 'Registered Nurse Paper 2' },
    { id: 'rm-paper-1', label: 'RM Paper 1', description: 'Registered Midwife Paper 1' },
    { id: 'rm-paper-2', label: 'RM Paper 2', description: 'Registered Midwife Paper 2' },
    { id: 'rphn-paper-1', label: 'RPHN Paper 1', description: 'Registered Public Health Nurse Paper 1' },
    { id: 'rphn-paper-2', label: 'RPHN Paper 2', description: 'Registered Public Health Nurse Paper 2' },
  ];
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (result.questions.length === 0) {
        setUploadStatus('error');
        setUploadMessage('No questions found in the document. Please check the formatting.');
        return;
      }

      setExtractedQuestions(result.questions);
      setUploadStatus('success');
      setUploadMessage(`Successfully extracted ${result.questions.length} questions from ${file.name}`);
      
    } catch (error) {
      console.error('Error parsing document:', error);
      setUploadStatus('error');
      setUploadMessage('Failed to parse document. Please check the file format and try again.');
    } finally {
      setUploading(false);
      // Reset file input
            ...
            <div className="space-y-4 text-blue-800">
              <div>
                ...
                <ul className="list-disc list-inside space-y-1 text-sm ml-6">
                  <li>Save document as <strong>.docx</strong> format</li>
                  <li>Start each question with "Question [number]:"</li>
                  <li>List options as A), B), C), D), etc.</li>
                  <li>Mark correct answer with <strong>**</strong> at the end</li>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
            ...
            <div className="bg-yellow-50 p-3 rounded">
              ...
              <p className="text-xl font-bold text-yellow-900">
                {(extractedQuestions.reduce((sum, q) => sum + q.options.length, 0) / extractedQuestions.length).toFixed(1)}
              </p>
            </div>
