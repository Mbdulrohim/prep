...
export default function EnhancedRMExamEntry({ examId, onStartExam }: EnhancedRMExamEntryProps) {
  ...
  const loadExamData = async () => {
    try {
      ...
      const eligibility = await rmUserAccessManager.canStartRMExam(user!.uid, examId);
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-green-50 via-white to-emerald-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <h2 className="text-xl font-semibold text-gray-900 mb-2">Loading RM Exam</h2>
      ...
      <div className="min-h-screen bg-gradient-to-br from-red-50 via-white to-red-50 flex items-center justify-center px-4">
        <div className="text-center max-w-md mx-auto">
          <div className="bg-white rounded-lg shadow-lg p-8">
            ...
            <h2 className="text-2xl font-semibold text-gray-900 mb-3">Unable to Start Exam</h2>
          ...
          <div className="bg-white rounded-lg shadow-sm p-6">
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <span className="font-medium text-gray-900">{exam.questionsCount}</span>
              </div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-lg shadow-sm p-6">
              <div className="flex items-center gap-3 mb-4">
                ...
                <h2 className="text-xl font-semibold text-gray-900">RM Access Active</h2>
              </div>
              ...
              {rmAccess && (
                <div className="space-y-2 text-sm text-gray-600">
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium capitalize">{rmAccess.accessMethod}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Granted:</span>
                    <span className="font-medium">{rmAccess.accessGrantedAt.toLocaleDateString()}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Max Attempts:</span>
                    <span className="font-medium">{rmAccess.adminSettings.maxAttempts}</span>
                  </div>
                </div>
              )}
            </div>
            ...
            {previousAttempts.length > 0 && (
              <div className="bg-white rounded-lg shadow-sm p-6">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <h2 className="text-lg font-semibold text-gray-900">Previous Attempts</h2>
                </div>
                ...
                <div className="space-y-3">
                  {previousAttempts.slice(0, 3).map((attempt, index) => (
                    <div key={index} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                      <div>
                        <p className="font-medium text-gray-900">Attempt {index + 1}</p>
                        <p className="text-sm text-gray-600">
                          {new Date(attempt.attemptDate).toLocaleDateString()}
                        </p>
                      </div>
                      <div className="text-right">
                        <p className="font-medium text-gray-900">{attempt.percentage}%</p>
                        <p className={`text-sm ${attempt.percentage >= 70 ? 'text-green-600' : 'text-red-600'}`}>
